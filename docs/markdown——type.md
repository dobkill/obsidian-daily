...
1. 创建或覆盖任务必须提供完整 `@YYYY-MM-DD HH:mm-HH:mm`。
2. `#项目：项目名` 表示后续任务归入该项目；项目不存在时自动创建。
3. `#项目：` 或 `#项目：未归属项目` 表示未归属任务。
4. 同项目同名且同日可匹配的任务会被覆盖；否则创建新任务。
...
| `done` | `done:2026-05-27,2026-05-29` | 标记已完成发生日期。 |

## 组合任务

组合任务必须有开始与结束时间。轻量子任务可以只写标题，也可以写自己的 `@HH:mm-HH:mm`；写时间时必须完全落在组合任务时间范围内。
